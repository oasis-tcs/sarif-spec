...
* Stefan: For software to describe findings, interpret, and act upon them there are different aspects:
  ...
  * vulnerability: As Jeff said interpretation of what it is so that the recipient of the information can act upon
  * mitigations: Are mitigations known and can the receipient of the information apply them
  * incidents: Actually exploited or how to grade the risk for the recipient
  * interests: Vendors / suppliers and customers are only aligned per minimal enforcable regulations competing brand damage (too late not the full information) with risk reduction (enable the recipient to assess their "own" risk)
  * correctness: Reliance on declared state (by someone on the internet: my project is MIT when the tree has GPL in it, outdated versions, ...)
