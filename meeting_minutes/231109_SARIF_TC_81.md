 ...
 * SARIF 2.2:
   * Mike provides reminders of existing SARIF properties, level, rank, default configuration
   ...
   * Katrina: Microsoft focus supports accuracy, confidence, probability, impact and others. A final aggregated number is stored in `result.rank`.
   * Yekaterina: there is a distinction between engine precision and rule precision for MicroFocus tooling
