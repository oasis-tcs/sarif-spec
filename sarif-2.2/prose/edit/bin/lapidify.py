...
def load_label_to_display_lut(
    path: Union[str, pathlib.Path] = SECTION_LABEL_TO_DISPLAY_AT,
) -> dict[str, str]:
...
def load_display_to_label_lut(
    path: Union[str, pathlib.Path] = SECTION_DISPLAY_TO_LABEL_AT,
) -> dict[str, str]:
...
def load_eg_label_to_global_lut(
    path: Union[str, pathlib.Path] = EG_LABEL_TO_GLOBAL_AT,
) -> dict[str, str]:
...
def load_eg_global_to_label_lut(
    path: Union[str, pathlib.Path] = EG_GLOBAL_TO_LABEL_AT,
) -> dict[str, str]:
            ...
            if line.startswith(tag) and clean_headings:
                ...
                if terse_line in APPENDIX_HEAD_REMAP:
                    ...
                    if "{" in line and line.count("{") == 3:
                        print("OBSCURE_FIX_NEEDED::::>incoming", terse_line)
                        print("OBSCURE_FIX_THIS::::>naive", line)
                        parts_found = line.split("{")
                        parts_kept = [parts_found[0]]
                        for part in parts_found[1:]:
                            if ".unnumbered" in part:
