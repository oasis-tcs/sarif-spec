...
* Expand the scope of our TC, keeping the SARIF name?
  ...
  * Katrina thinks, the two types of analysis are different to an extent to over complicated any spec that tries to describe both on low level
...
* Create a new TC with a new name?
  ...
  * Stefan sees the proposal from Katrina as promising candidate to not disrupt but also standardize static and dynamic analysis interchange best
