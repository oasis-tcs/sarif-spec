...
* Paul Anderson will lose voting status if he does not join before the end of this meeting
...
* Jeff Williams has declared a leave of absence until the first meeting in November
...
* Michael:
  * Agrees to not disrupt teh existing format too much without clear benefits for existing and new users of SARIF
...
* Jeff:
  ...
  * SBOM approaches organize around components providing different perspectives with massive referencing
    * Without balancing the impact i.e. not making a more comprehensible information item
      * Interpretatbility degrades when unweighted external references grow in numbers
  * Suggests a more balanced appraoch that adds evidence only is added to increase the comprehensibility
* Stefan: 
  * Not convinced if SBOMs or Security Advisories will contain SARIF dta but instead wil point to public reports (CVEs) as SARIF is more for intermediate states.
  * CVEs may reference SARIF data to prove the point they make (similarly to Proof of concepts)
* Aditya:
  * Supply chain analysis you cannot run in isolation
  * SARIF is more for the consumer but who would produce the SARIF
* Nathan:
  * This is what his team provides as service coming out of crashes (producers)
  * Is a SARIF log ony about the errors we find or can it deliver assurance like 
